...
export const validateCnpj = cnpj => {
  ...
  const numbers = cnpj.length - 2;
  const string = cnpj.substring(numbers);
  const checker1 = +string.charAt(0);
  const checker2 = +string.charAt(1);
...
export const removeCpfMask = value => value && value.replace(/\./g, '').replace('-', '');
